  ...
  describe "when validating values" do
    describe "for name" do
      it "should support an alphanumerical name" do
        described_class.new(:name => 'test', :ensure => :present, :value => 'value')[:name].should == 'test'
      end

      it "should support underscores" do
        described_class.new(:name => 'test_property', :ensure => :present, :value => 'value')[:name].should == 'test_property'
      end
   
      it "should support hyphens" do
        described_class.new(:name => 'test-property', :ensure => :present, :value => 'value')[:name].should == 'test-property'
      end

      it "should not support spaces" do
        expect { described_class.new(:name => 'test value', :ensure => :present, :value => 'value') }.to raise_error(Puppet::Error, /test value is not a valid system property./)
      end
    end

    describe "for ensure" do
      it "should support present" do
        described_class.new(:name => 'test', :ensure => 'present', :value => 'value')[:ensure].should == :present
      end

      it "should support absent" do
        described_class.new(:name => 'test', :ensure => 'absent', :value => 'value')[:ensure].should == :absent
      end

      it "should not support other values" do
        expect { described_class.new(:name => 'test', :ensure => 'foo', :value => 'value') }.to raise_error(Puppet::Error, /Invalid value "foo"/)
      end

      it "should not have a default value" do
        described_class.new(:name => 'test', :value => 'value')[:ensure].should == nil
      end
    end
  ...
  describe "when autorequiring" do     
    describe "user autorequire" do
      ...
      it "should not autorequire a user when no matching user can be found" do
        catalog.add_resource sysprop
        sysprop.autorequire.should be_empty
      end
  
      it "should autorequire a matching user" do
        catalog.add_resource sysprop
        catalog.add_resource user
        reqs = sysprop.autorequire
        reqs.size.should == 1
        reqs[0].source.ref.should == user.ref
        reqs[0].target.ref.should == sysprop.ref
      end
    end
