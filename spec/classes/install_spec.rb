...
describe 'glassfish::install' do
  ...
  context 'on a RedHat OSFamily' do
    ...
    describe 'with a custom download URL' do
      # Set the osfamily fact
      let(:facts) { {
        :osfamily => 'RedHat'
      } }
      
      # Include required classe with custom download URL 
      let(:pre_condition) { 'class {"glassfish": 
        download_mirror => "http://test.local/glassfish-mirror"}' 
      }
    ...
    describe 'with glassfish_version => 4.0' do
      # Set the osfamily fact
      let(:facts) { {
        :osfamily => 'RedHat'
      } }
